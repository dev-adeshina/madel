...
class ApiClient 
{
    ...
    public function get(string $url, mixed $query = [], array $headers = []): Response 
    {
        return $this->get->execute($url, $query, $headers);
    }

    public function post(string $url, mixed $data = [], array $headers = []): Response 
    {
        return $this->post->execute($url, $data, $headers);
    }

    public function patch(string $url, mixed $data = [], array $headers = []): Response 
    {
        return $this->patch->execute($url, $data, $headers);
    }

    public function put(string $url, mixed $data = [], array $headers = []): Response
    {
        return $this->put->execute($url, $data, $headers);
    }

    public function delete(string $url, mixed $query = [], array $headers = []): Response 
    {
